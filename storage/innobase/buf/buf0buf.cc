...
#ifdef UNIV_NVDIMM_CACHE
/** Initialize a NVDIMM buffer pool instance.
@param[in]	buf_pool	    NVDIMM buffer pool instance
@param[in]	buf_pool_size size in bytes
@param[in]	instance_no   id of the instance
@param[in,out]  mutex     Mutex to protect common data structures
@param[out] err           DB_SUCCESS if all goes well */
static void nvdimm_buf_pool_create(buf_pool_t *buf_pool, ulint buf_pool_size,
                            ulint instance_no, std::mutex *mutex,
                            dberr_t &err) {
  ulint i;
  ulint chunk_size;
...
#ifdef UNIV_NVDIMM_CACHE
/** Frees the NVDIMM buffer pool global data structures. */
static void nvdimm_buf_pool_free() {
  ut_free(nvdimm_buf_pool_ptr);
  nvdimm_buf_pool_ptr = nullptr;
}
#endif /* UNIV_NVDIMM_CACHE */
...
@param[in]  total_size    Size of the total pool in bytes.
@param[in]  n_instances   Number of NVDIMM buffer pool instances to create.
@return DB_SUCCESS if success, DB_ERROR if not enough memory or error */
dberr_t nvdimm_buf_pool_init(ulint total_size, ulint n_instances) {
  ulint i;
  const ulint size = total_size / n_instances;

  ut_ad(n_instances > 0);
  ut_ad(n_instances <= MAX_BUFFER_POOLS);
  ut_ad(n_instances == srv_nvdimm_buf_pool_instances);

  nvdimm_buf_pool_ptr =
      (buf_pool_t *)ut_zalloc_nokey(n_instances * sizeof *nvdimm_buf_pool_ptr);
  ...
  for (i = 0; i < n_instances; /* no op */) {
    ...
    for (ulint id = i; id < n; ++id) {
      threads.emplace_back(std::thread(nvdimm_buf_pool_create, &nvdimm_buf_pool_ptr[id], size,
                                       id + srv_buf_pool_instances, &m, std::ref(errs[id])));
    }
    ...
    if (err != DB_SUCCESS) {
      for (size_t id = 0; id < n; ++id) {
        if (nvdimm_buf_pool_ptr[id].chunks != nullptr) {
          buf_pool_free_instance(&nvdimm_buf_pool_ptr[id]);
        }
      }
...
#ifdef UNIV_NVDIMM_CACHE
  buf_pool_invalidate_instance(&nvdimm_buf_pool_ptr[0]);
#endif /* UNIV_NVDIMM_CACHE */
}
